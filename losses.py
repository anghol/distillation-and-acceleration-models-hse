...
class LossSummary:
    ...
    def get_loss_description(self, show_g=True, show_d=True):
        description_g = ''
        description_d = ''
        
        if show_g:
            avg_adv, avg_gram, avg_color, avg_content = self.avg_loss_G()
            description_g += f'loss G: adv {avg_adv:2f} con {avg_content:2f} gram {avg_gram:2f} color {avg_color:2f}'

        if show_d:    
            avg_adv_d = self.avg_loss_D()
            description_d += f'loss D: {avg_adv_d:2f}'

        if show_g and show_d:
            return f'{description_g} | {description_d}'
        else:
            return description_g + description_d
